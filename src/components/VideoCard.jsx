...
function VideoCard(props) {
  ...
  return (
    <div style={{ width: props.width, display: props.display }} className='w-[100%] sm:w-[90%] md:w-[100%] relative cursor-pointer videoComponent'>
      <img onClick={() => pageRoute(`/watch/${props.videoId}`)} className='w-[100%] max-w-[320px] rounded-[12px] videoImage' src={props.thumbnail} />
      <div style={{ width: props.rightWidth }} className='flex w-[100%] gap-x-3 items-start mt-2'><div>
        <h3 onClick={() => props.id.videoId ? pageRoute(`/watch/${props.id?.videoId}`) : pageRoute(`/channel/${props.id?.channelId}`)} className='text-[14px] lg:text-[16px] font-semibold leading-[20px] w-[100%] max-w-[320px]'>
